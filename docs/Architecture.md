...
| `pages/dashboard.py` | `web/pages/dashboard.py` | Implemented | Two-panel research dashboard. Left panel: filter controls (query search, model multi-select, date range, experiment ID), sort selector (date/tokens/cost/rounds), and JSON/CSV export buttons. Right panel: transcript table with single-row selection that opens a detail view with the full transcript, convergence chart, cost breakdown, influence heatmap, and cumulative cost chart. Reactive filtering — controls update the table automatically. |
| `components/transcript_browser.py` | `web/components/transcript_browser.py` | Implemented | Pure-Python filter and sort functions for transcript summaries. `filter_transcripts()` applies query text (case-insensitive), model alias, date range, and experiment ID filters. `sort_transcripts()` orders by any summary field with None-safe handling. |
| `components/charts/convergence.py` | `web/components/charts/convergence.py` | Implemented | Convergence visualization. `compute_convergence()` measures per-model response change between consecutive rounds using `difflib.SequenceMatcher` word-level similarity (0=identical, 1=completely different). `render_convergence_chart()` renders a grouped bar chart via `ui.echart()`. |
| `components/charts/cost.py` | `web/components/charts/cost.py` | Implemented | Cost visualizations. `per_debate_cost()` extracts per-model cost from `transcript.metadata["stats"]["per_model"]`. `cumulative_cost_series()` computes running totals across transcript summaries. `render_per_debate_chart()` and `render_cumulative_chart()` render bar and line charts via `ui.echart()`. |
| `components/charts/influence.py` | `web/components/charts/influence.py` | Implemented | Influence heatmap. `compute_influence()` builds an NxN matrix where cell (i,j) is the average change in model j's response across reflection rounds where model i was present. `render_influence_heatmap()` renders a colored heatmap via `ui.echart()`. |
| `components/export.py` | `web/components/export.py` | Implemented | Transcript export. `export_json()` serializes filtered summaries as pretty-printed JSON. `export_csv()` produces a CSV with fixed column order (short_id, date, query, panel, synthesizer, rounds, tokens, cost, experiment_id). |
